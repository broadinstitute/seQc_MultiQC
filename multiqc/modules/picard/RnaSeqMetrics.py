                    ...
                    if 'PF_BASES' in keys and 'PF_ALIGNED_BASES' in keys:
                        for s_name in s_names:
                            self.picard_RnaSeqMetrics_data[s_name]['PF_NOT_ALIGNED_BASES'] = \
                                self.picard_RnaSeqMetrics_data[s_name]['PF_BASES'] - self.picard_RnaSeqMetrics_data[s_name][
                                    'PF_ALIGNED_BASES']

            if s_names is not None and 'normalized_position	All_Reads.normalized_coverage' in l:
                for s_name in s_names:
                    self.picard_RnaSeqMetrics_histogram[s_name] = dict()
                in_hist = True
